...
### Model content and inputs from public data

#### Summary

| Element | Model content | Input |
| - | - | - |
| Arrivals and response category | Homogeneous Poisson Process | Mean inter-arrival time by response category |
| Resource | Single pooled ambulance resource | Number of ambulances available (assuming 24/7 pool) |
| Queue | FIFO | - |
| Response time (mobilisation time + time to scene) | Lognormal distribution* | Mean and SD by response category |
| On scene time | Deterministic | Mean |
| Travel time to hospital | Exponential distribution | Mean |
| Handover time | Lognormal distribution | Mean and SD |
| Wrap up time | Deterministic | Mean |

#### Arrivals and response category
...
A first-in-first-out (FIFO) queue is used - a simple approach, common in the ambulance DES literature.
...
Travel time to hospital is currently very rough.
