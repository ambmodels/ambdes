...
This stage models the whole trust.
...
pd.read_csv("stage1/data/arrivals.csv", index_col=0)
```
...
As a placeholder until we receive internal data to support this, we are using estimates based on @roberts_focus_2014, which reports:
...
* Change to synthetic data, lognormal, vary by C1 to C4.
* Add code to check assumption: travel to scene varies by response category.
* Then would require on real data:
    * Assess whether travel to scene time varies by response category.
...
As a placeholder, have a deterministic time from @davis_what_2025, a London study reporting times from 2023:

* Mean on-scene time: **44 minutes**
* Median on-scene time: 46 minutes
* Inter-quartile range (IQR) for on-scene time: 33 to 54 minutes

On-scene time is currently deterministic. Ideally a lognormal distribution would be used (as in [AmbModelOpen](https://github.com/nhsengland/AmbModelOpen/), which uses empirical or lognormal for time at scene).

#### Next steps

* Change to synthetic data, lognormal, vary by C1 to C4.
* Add code to check assumption: on-scene time varies by response category.
* Then would require on real data:
    * Assess whether on-scene time varies by response category.
...
Current placeholder is lognormal with mean and standard deviation based on AmbSYS data reporting mean and 90th percentile handover time by region and month (A142, A143).
...
Current placeholder is determinstic from @davis_what_2025 (London, 2023):
...
TBC which inputs county-specific and which not.
